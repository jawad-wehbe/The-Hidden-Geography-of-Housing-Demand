...
la_path <- file.path(shapefiles_root, "LA/LAD_DEC_2023_UK_BFE.shp")   # CONFIG
...
la_target_path <- file.path(project_root, "produced/gap/la_targets_gaps_for_map_updated.csv")   # CONFIG


oa_in_la_output_path <- file.path(project_root, "temp/oa_to_la.gpkg")   # CONFIG

bristol_path <- file.path(shapefiles_root, "Other/Bristol_boundary.geojson")   # CONFIG

out_dir <- file.path(project_root, "produced/reclassification/")   # CONFIG

########################
# reading the data in 
########################

if (!use_demo) bristol_boundary <- st_read(bristol_path) %>%   # DEMO: the corrected Bristol boundary is only needed for the real run
  select(geometry) %>%
  st_transform(27700)
...
if (!use_demo) la$geometry[la$la_name == "Bristol"] <- bristol_boundary$geometry[1]   # DEMO: Bristol is not in the demo
...
if (use_demo) la_list <- c("Calderdale")   # DEMO: the demo dataset covers Calderdale only
...
for (la_test in la_list) {
 ...
 all_points <- bind_rows(town_points, suburb_points %>% slice_sample(n= 10))
  ...
  if (nrow(all_points) == 0) {   # CONFIG (bug fix): if every OSM place query failed (Overpass throttling), tmap cannot draw an empty layer; skip the labels instead of crashing
    message("No OSM place labels available (Overpass returned nothing); drawing the map without labels")
  } else if (length(uniq_sizes) == 1) {
    tm <- tm +
      tm_shape(all_points) +
      tm_text(
        "name",
  ...
  # print(tm)   # DEMO 2026-09-15 (Jawad): print commented out; under Rscript it wrote the figure to a stray Rplots.pdf. The figure is saved right below.
