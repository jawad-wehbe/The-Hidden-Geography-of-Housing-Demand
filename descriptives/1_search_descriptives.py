...
# Paths
db_path = SEARCH_FILTERED_DB   # CONFIG
spill_dir = SEARCH_ROOT / "temp/duckdb_spill"   # CONFIG


# Path to old duckdb (used only to get the listings)
old_search_path = SEARCH_PROCESSED_DB   # CONFIG

#output table results

output_dir = PROJECT_ROOT / "output/tables/hidden_geography_of_housing_demand"   # CONFIG

# search location lookup path

location_lookup_path = SEARCH_RAW_DIR / "search_location_lookup.csv"   # CONFIG
...
for label, condition in conditions.items():
    ...
    db.execute(f"PRAGMA threads={N_THREADS};")   # CONFIG
    db.execute(f"SET memory_limit='{DUCKDB_MEMORY}';")   # CONFIG
...
# sales
with duckdb.connect(old_search_path, read_only=True) as con:
    con.execute(f"SET temp_directory='{spill_dir}';")
    con.execute(f"SET memory_limit='{DUCKDB_MEMORY}';")   # CONFIG
    con.execute(f"PRAGMA threads={N_THREADS};")   # CONFIG
...
final_table[["Buying", "Letting", "Combined"]] = (
    ...
    .round(0)
    .astype("Int64")
    .map(lambda x: f"{x:,}" if pd.notnull(x) else "")   # CONFIG (compat): DataFrame.applymap was removed in pandas 3, DataFrame.map is the replacement
)
