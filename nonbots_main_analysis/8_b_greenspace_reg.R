...
# transaction
transactions_path <- file.path(raw_root, "HMLR/pp-complete.csv")   # CONFIG

postcode_to_coord_path <- file.path(shapefiles_root, "ONSPD_FEB_2025/Data/ONSPD_FEB_2025_UK.csv")   # CONFIG

# other paths

building_addressbook_path <- file.path(shapefiles_root, "OS/AddressBasePremium_FULL_2025-05-09_001.gpkg")   # CONFIG

oa_path <- file.path(shapefiles_root, "output_areas_2021.gpkg")   # CONFIG


duckdb_path <- file.path(project_root, "temp/hidden_geography_of_housing_demand/duckcb_greenspace.duckdb")   # CONFIG

temp_path <- file.path(project_root, "temp/hidden_geography_of_housing_demand/")   # CONFIG

#fig_dir <- "/home/jawad/Desktop/Projects/housing_targets/output/figures/hidden_geography_of_housing_demand/"
fig_dir <- file.path(project_root, "output/figures/hidden_geography_of_housing_demand/nonbots/")   # CONFIG (bug fix: fig_dir was commented out but is used by ggsave near line 215; same folder as 8_c figures)

spill_dir <- file.path(project_root, "temp/hidden_geography_of_housing_demand/spill_dir/")   # CONFIG

dataset_path <- file.path(project_root, "temp/hidden_geography_of_housing_demand/transactions_greenspace.parquet")   # CONFIG

cw_dir <- file.path(shapefiles_root, "Crosswalks/")   # CONFIG

searches_path <- file.path(project_root, "produced/hidden_geography_of_housing_demand/nonbots/all/oa_weekly_search_with_geos.parquet")   # CONFIG

weekly_reg_dataset_path <- file.path(project_root, "produced/hidden_geography_of_housing_demand/nonbots/regression_datasets/greenspace_weekly_reg_dataset_complete.parquet")   # CONFIG

monthly_reg_dataset_path <- file.path(project_root, "produced/hidden_geography_of_housing_demand/nonbots/regression_datasets/greenspace_monthly_reg_dataset_complete.parquet")   # CONFIG

results_dir <- file.path(project_root, "produced/hidden_geography_of_housing_demand/nonbots/regression_results/")   # CONFIG
...
# Memory limit 
dbExecute(con, paste0("SET memory_limit='", duckdb_memory, "';"))   # CONFIG

# cores
dbExecute(con, paste0("SET threads=", n_threads, ";"))   # CONFIG
...
# DEMO 2026-09-11: greenspace percentile plot commented out (Jawad: not needed)
# pctile_df <- tibble(
...
rm(list = setdiff(ls(), c("reg_df", "fig_dir", "results_dir", "scale_per_100m2", "monthly_reg_dataset_path", "weekly_reg_dataset_path", "n_threads",
                          "weekly_searches", "greenspace_df", "transactions_oa", "transaction", "postcode_lookup", "test_with_coords",
                          "cw_oa_lsoa", "cw_oa_msoa", "cw_oa_ttwa")))   # CONFIG (bug fix): keep n_threads for setFixest_nthreads and the objects the monthly block below still uses

gc()

setFixest_nthreads(n_threads)   # CONFIG


# specs: outcome column, base var (for ybar), transform type, whether to drop y == 0, y-axis label
# DEMO 2026-09-11: only weekly log_search kept (Jawad); ihs_trans and log_price rows commented, trailing comma removed.
specs <- tribble(
...
# specs: outcome column, base var (for ybar), transform type, whether to drop y == 0, y-axis label
# DEMO 2026-09-11: only monthly ihs_trans and log_price kept (Jawad); log_search row commented.
