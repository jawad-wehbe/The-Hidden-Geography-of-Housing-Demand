...
select <- dplyr::select   # CONFIG (bug fix): library(multcomp) attaches MASS, whose select() masks dplyr::select() and breaks the pipes below
gc()
...
# land paths
landuse1_path <- file.path(shapefiles_root, "NGD/landuse_1.gpkg")   # CONFIG

landuse2_path <- file.path(shapefiles_root, "NGD/landuse_2.gpkg")   # CONFIG

landuse3_path <- file.path(shapefiles_root, "NGD/landuse_3.gpkg")   # CONFIG

landuse4_path <- file.path(shapefiles_root, "NGD/landuse_4.gpkg")   # CONFIG

# land to uprn lookup tables

land_uprn_1_path <- file.path(shapefiles_root, "NGD/site-to-address-reference-01.csv")   # CONFIG

land_uprn_2_path <- file.path(shapefiles_root, "NGD/site-to-address-reference-02.csv")   # CONFIG

land_uprn_3_path <- file.path(shapefiles_root, "NGD/site-to-address-reference-03.csv")   # CONFIG

land_uprn_4_path <- file.path(shapefiles_root, "NGD/site-to-address-reference-04.csv")   # CONFIG
...
os_zoomstack_path <- file.path(shapefiles_root, "OS/OS_Open_Zoomstack.gpkg")   # CONFIG
...
land_sample_path <- file.path(project_root, "temp/hidden_geography_of_housing_demand/land_sample.gpkg")   # CONFIG
...
SELECT 
  ST_MakeValid(geom)::GEOMETRY AS valid_geom,   -- CONFIG (bug fix): DuckDB spatial >= 1.5 tags geometry with its CRS; the cast stores plain GEOMETRY so the table writes in the default storage format and the RTREE index below accepts it. No-op on older versions.
  *
EXCLUDE (geom)
FROM ST_Read('%s');
", land_use_file))
...
SELECT 
  ST_MakeValid(geom)::GEOMETRY AS valid_geom,   -- CONFIG (bug fix): see land_use
  *
EXCLUDE (geom)
FROM ST_Read('%s');
", buildings_file))
