...
source(file.path(here(), "config.R"))   # CONFIG (placed after the rm() so the config survives): all data roots come from config.R (real or demo mode)

# define paths

base_data_dir <- path.expand(file.path(raw_root, "NHBC"))   # CONFIG
out_dir <- file.path(project_root, "produced/search_on_build_shock/setup")   # CONFIG


gpkg_path <- file.path(
  path.expand(shapefiles_root),   # CONFIG
...
site_weekly_oa <- site_weekly_oa %>% st_drop_geometry()   # DEMO 2026-09-11 bug fix: result was not assigned, so write_parquet received an sf geometry column (fails with arrow >= 17)
