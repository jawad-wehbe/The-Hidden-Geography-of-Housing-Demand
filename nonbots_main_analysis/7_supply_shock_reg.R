...
all_searches_path <- file.path(project_root, "produced/hidden_geography_of_housing_demand/nonbots/all/oa_weekly_search_with_geos.parquet")   # CONFIG
...
results_dir <- file.path(project_root, "produced/hidden_geography_of_housing_demand/nonbots/all/")   # CONFIG
...
rm(list = setdiff(ls(), c("stacked_df_reg", "fig_dir", "results_dir", "buying_tidy_path", "buying_below_median_path",
                          "buying_above_median_path", "buying_new_rural_path", "all_searches_path", "oa_path", "n_threads", "use_demo")))   # CONFIG (bug fix): keep the paths and settings used below the wipe (use_demo: DEMO y-axis switch)
gc()
...
# CONFIG (bug fix): stacked_df was removed at line 141, so these exploratory stats cannot run; commented out
...
# CONFIG (bug fix): the block below references objects removed earlier in this script (transaction, postcode_lookup, stacked_df_updated) and cannot run; commented out
