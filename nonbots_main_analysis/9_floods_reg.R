...
floods_england_path <- file.path(shapefiles_root, "Other/Floods/NRW_HISTORIC_FLOODMAP.gpkg")   # CONFIG

floods_wales_path <- file.path(shapefiles_root, "Other/Floods/Recorded_Flood_Outlines.gpkg")   # CONFIG

oa_path <- file.path(shapefiles_root, "output_areas_2021.gpkg")   # CONFIG

output_path <- file.path(project_root, "produced/hidden_geography_of_housing_demand/")   # CONFIG

ttwa_oa_path <- file.path(shapefiles_root, "Crosswalks/oa_ttwa.parquet")   # CONFIG
...
weekly_path <- file.path(project_root, "produced/hidden_geography_of_housing_demand/nonbots/all/oa_weekly_search_with_geos.parquet")   # CONFIG

# Outputs
reg_dataset_weekly_path <-   file.path(project_root, "produced/hidden_geography_of_housing_demand/nonbots/regression_datasets/floods_weekly_reg_dataset_complete.parquet")   # CONFIG

reg_dataset_monthly_path <- file.path(project_root, "produced/hidden_geography_of_housing_demand/nonbots/regression_datasets/floods_monthly_reg_dataset_complete.parquet")   # CONFIG
...
# DEMO 2026-09-11: only weekly search_log kept (Jawad); trans_ihs row commented.
specs <- tribble(
...
for (r in seq_len(nrow(specs))) {
  ...
  # print(p)   # DEMO 2026-09-15 (Jawad): print commented out; under Rscript it wrote the figure to a stray Rplots.pdf. The figure is saved right below.
...
# specs: outcome column, base var (for ybar + filtering), transform, drop zeros, labels
# DEMO 2026-09-11: only monthly trans_ihs and price_log kept (Jawad); search_log row commented, trailing comma removed.
