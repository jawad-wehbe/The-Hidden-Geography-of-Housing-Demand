...
# paths
ed_path <- file.path(project_root, "produced/hidden_geography_of_housing_demand/nonbots/all/ed_reg_dataset.parquet")   # CONFIG
sd_path <- file.path(project_root, "produced/hidden_geography_of_housing_demand/nonbots/all/sd_reg_dataset.parquet")   # CONFIG
saved_or_contacted_searches_path <- file.path(project_root, "produced/hidden_geography_of_housing_demand/nonbots/saved_or_contacted/oa_weekly_search_with_geos.parquet")   # CONFIG
fig_dir <- file.path(project_root, "output/figures/hidden_geography_of_housing_demand/nonbots/saved_or_contacted/")   # CONFIG

# where to save regression output so we don't rerun feols
results_dir <- file.path(project_root, "produced/hidden_geography_of_housing_demand/nonbots/saved_or_contacted/")   # CONFIG
...
reg_dataset_dir<- file.path(project_root, "produced/hidden_geography_of_housing_demand/nonbots/regression_datasets/")   # CONFIG
...
# clear for ram
rm(new_searches, saved_or_contacted_searches, weekly); gc()
...
rm(list = setdiff(ls(), c("stacked_df_reg", "fig_dir", "buying_tidy_path", "use_demo")))   # DEMO: keep use_demo for the y-axis switch below
gc()
...
buying_plot <- ggplot(
  ...
  scale_y_continuous(
    breaks = if (use_demo) waiver() else seq(-1, 2.2, by = 0.4)   # DEMO: fixed breaks suit the real estimates only; demo lets ggplot choose
  ) +
  theme_classic() +
  theme(
...
# print(buying_plot)   # DEMO 2026-09-15 (Jawad): print commented out; under Rscript it wrote the figure to a stray Rplots.pdf. The figure is saved right below.
...
# CONFIG (bug fix): stacked_df was removed at the wipe above, so these exploratory stats cannot run; commented out
...
# DEMO 2026-09-11: below-median subsample regression commented out (Jawad: not needed). Transactions section below is unchanged.
# ######################################################################################################
...
# CONFIG (bug fix): the block below references objects removed at the wipe above (transaction, postcode_lookup, stacked_df_updated) and cannot run; commented out
